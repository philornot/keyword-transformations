...
function validateQuestion(q: QuestionInput, index: number): void {
    ...
    if (![3, 4, 5].includes(q.maxWords)) throw error(400, `Q${index + 1}: maxWords must be 3, 4, or 5.`);
}
...
function insertQuestions(setId: number, questions: QuestionInput[]): void {
    ...
    for (const [i, q] of questions.entries()) {
        stmt.run(setId, i + 1, q.sentence1.trim(), q.sentence2WithGap.trim(), q.keyword.trim().toUpperCase(), q.correctAnswer.trim(), JSON.stringify((q.alternativeAnswers ?? []).map((a) => a.trim()).filter(Boolean)), JSON.stringify((q.exampleWrongAnswers ?? []).map((a) => a.trim()).filter(Boolean)), q.maxWords,);
    }
}
...
export const PUT: RequestHandler = async ({request, params, cookies}) => {
    ...
    if (isAdmin) {
        db.transaction(() => {
            ...
            // Answers reference question rows via a FK. We must delete answers
            // first (through their attempt join) before dropping the questions,
            // otherwise SQLite raises a FOREIGN KEY constraint error.
    ...
    // ── User fork ───────────────────────────────────────────────────────
    // Always diff against the *origin* set (follow parent chain once).
    ...
    if (originSet) {
        ...
        if (fingerprint(title, sourceLabel, questions) === fingerprintFromDb(originSet, originQRows)) {
            // Identical to origin — no fork needed.
            return json({slug: originSet.slug, isNew: false});
        }
    }

    // Content differs — persist a new forked set (never public by default).
